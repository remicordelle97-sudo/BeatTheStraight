      ...
      if (type === 'repair') {
        ...
        socket.emit('upgrade_ship', { shipId: sid, type: 'repair', health: _health }, (res) => {
          if (res?.success) {
            ...
            if (st) { st.totalDamage = 0; const shipData = getSelectedShipData(); if (shipData) st.speed = shipData.speed || st.speed; }
...
document.getElementById('scp-repair').addEventListener('click', () => {
  ...
  socket.emit('upgrade_ship', { shipId: sid, type: 'repair', health: currentHealth }, (res) => {
    if (res?.success) {
      const st = shipStates[sid];
      if (st) { st.totalDamage = 0; const s2 = getShipData(sid); if (s2) st.speed = s2.speed || st.speed; }
    ...
    btn.addEventListener('click', () => {
      ...
      socket.emit('upgrade_ship', { shipId: sid, type: 'repair', health: currentHealth }, (res) => {
        if (res?.success) {
          ...
          if (freshSt) { freshSt.totalDamage = 0; const freshShip = getShipData(sid); if (freshShip) freshSt.speed = freshShip.speed || freshSt.speed; }
